...
from VGSolvers import SolverGlobalFBSDE, SolverMultiStepFBSDE,SolverSumLocalFBSDE, SolverGlobalMultiStepReg, SolverGlobalSumLocalReg, SolverOsterleeFBSDE
...
#for method in ['Global']:
    # math model
    ##########################
    mathModel = VGmodel(T, N, r, theta, kappa, sigmaJ, K, x0)
...
ax.plot(PidePrice*np.ones(num_epochExt), label = 'Y0 Pide Price', linestyle = 'dashed')
ax.grid()
plt.legend()
plt.show()
