...
from SolversCoupledV import SolverSumLocalFBSDE
...
class Net( tf.keras.Model):
    def __init__( self,  nbNeurons, activation= tf.nn.tanh):
        super().__init__()
        self.nbNeurons = nbNeurons
        self.name_ = "FeedForwardANd0"
        self.ListOfDense =  [layers.Dense( nbNeurons[i],activation= activation, kernel_initializer= tf.keras.initializers.GlorotNormal())  for i in range(len(nbNeurons)) ]+[layers.Dense(2, activation= None, kernel_initializer= tf.keras.initializers.GlorotNormal())]
...
saveFig = "SUMLOCALVALEUR_BSize_"+str(batchSize)+"_lam_"+str(lam)+ "_N_"+str(N) +"_"
# DL model
##########################
if activation == 'tanh':
    activ = tf.nn.tanh
...
kerasModelUZ = Net( layerSize, activation)

# solver
#########################
solver = SolverSumLocalFBSDE(mathModel,kerasModelUZ, lRateY0)
