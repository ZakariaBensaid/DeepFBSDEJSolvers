...
class SolverGlobalFBSDE(SolverBase):
    ...
    def train(self,  batchSize,  batchSizeVal, num_epoch, num_epochExt):
        ...
        def optimizeBSDE( nbSimul):
            ...
            for istep in range(self.mathModel.N):
                ...
                # get back U, Z, 
                Z0, Gam, compens = self.modelKeras(self.mathModel.getStates(dN, listJumps, gaussJ))
                # target
                Y = Y - self.mathModel.dt* self.mathModel.f(Y) + Z0*dW + Gam - compens*self.mathModel.dt 
                # next t step
    ...
    def simulateGlobalErr(self, nbSimul):
        ...
        for istep in range(self.mathModel.N):
            ...
            # target
            Y = Y - self.mathModel.dt* self.mathModel.f(Y) + Z0* dW + Gam*(dN*gaussJ-compens)
            #Go to next step                 
            self.mathModel.oneStepFrom(dW, gaussJ)
...
class SolverMultiStepFBSDE(SolverBase):
    ...
    def train(self,  batchSize,  batchSizeVal, num_epoch, num_epochExt):
        ...
        def optimizeBSDE( nbSimul):
            ...
            for istep in range(self.mathModel.N):
                ...
                dW =  np.sqrt(self.mathModel.dt)*gaussian 
                # Adjoint variables 
                Y, Z0, Gam, compens = self.modelKeras(self.mathModel.getStates(dN, listJumps, gaussJ))
                # target
        ...
        for iout in range(num_epochExt):
            ...
            dN, listJumps, gaussJ  = self.mathModel.jumps()
            Y0 = self.modelKeras(self.mathModel.getStates(dN, listJumps, gaussJ))[0][0]
    ...
    def simulateGlobalErr(self, nbSimul):
        ...
        for istep in range(self.mathModel.N):
            ...
            dW =  np.sqrt(self.mathModel.dt)*gaussian 
            # Adjoint variables 
            Y, _, _, _ = self.modelKeras(self.mathModel.getStates(dN, listJumps, gaussJ))
            #costfunc
            costFunc += self.mathModel.dt* self.mathModel.f(Y)
            #Go to next step                 
...
class SolverSumLocalFBSDE(SolverBase):
    ...
    def train(self,  batchSize,  batchSizeVal, num_epoch, num_epochExt):
        ...
        def optimizeBSDE( nbSimul):
            ...
            errorComp = 0.
            #init val
            YPrev, Z0Prev, GamPrev, compens = self.modelKeras(self.mathModel.getStates(dN, listJumps, gaussJ))
            for istep in range(self.mathModel.N):
                # increment
                gaussian = tf.random.normal([nbSimul])
                dW =  np.sqrt(self.mathModel.dt)*gaussian 
                ...
                if (istep == (self.mathModel.N-1)):
                    ...
                    YNext, Z0Prev, GamPrev, compens = self.modelKeras(self.mathModel.getStates(dN, listJumps, gaussJ))
...
class SolverGlobalSumLocalReg(SolverBase):
    ...
    def train( self,  batchSize,  batchSizeVal, num_epoch, num_epochExt):
        ...
        def regressOptim( nbSimul):
            ...
            error = 0.
            # get back Y
            YPrev, = self.modelKeras(self.mathModel.getStates(dN, listJumps, gaussJ))
            for istep in range(self.mathModel.N):
                # target
                ...
                if (istep == (self.mathModel.N-1)):
                    ...
                    YNext, = self.modelKeras(self.mathModel.getStates(dN, listJumps, gaussJ))
...
class SolverGlobalMultiStepReg(SolverBase):
    ...
    def train( self,  batchSize,  batchSizeVal, num_epoch, num_epochExt):
        ...
        def regressOptim( nbSimul):
            ...
            for istep in range(self.mathModel.N): 
                # get back Y
                Y, = self.modelKeras(self.mathModel.getStates(dN, listJumps, gaussJ))
                # listforward
                listOfForward.append(Y)                 
                # to Add
        ...
        for iout in range(num_epochExt):
            ...
            self.mathModel.init(1)
            dN, listJumps, gaussJ  = self.mathModel.jumps()
            Y0 = self.modelKeras(self.mathModel.getStates(dN, listJumps, gaussJ))[0][0].numpy()
    ...
    def train(self,  batchSize,  batchSizeVal, num_epoch, num_epochExt):
        ...
        def optimizeBSDE( nbSimul):
            ...
            for istep in range(self.mathModel.N):
                ...
                dN, listJumps, gaussJ  = self.mathModel.jumps()
                # get back Y, Z, Gam
                Ynet, Z0, Gam, compens = self.modelKeras(self.mathModel.getStates(dN, listJumps, gaussJ))
                # adjoint variables
