...
parser.add_argument('--jumpFac',type=float, default =12)
parser.add_argument('--nbDays',type=int, default =1)
parser.add_argument('--lRateY0',type=float, default =0.01)
parser.add_argument('--lRateLoc',type=float, default =0.001)
parser.add_argument('--lRateReg',type=float, default =0.005)
...
###########################
mathModel0 = ModelCoupledFBSDE(T, QAver, R0, jumpFactor, alpha, beta, coeffOU, A, K, 0.5, 0, 0, 0, 0, theta,C, S0, h1, h2,sig0, sig, alphaTarget, jumpModel, 1)
...
for j in range(NbSimulation):
    ...
    #intensity alone
    #############################################
    ax[1,0].plot(solutionPlayer1.t*solutionPlayer1.dt*24, solutionPlayer1.lam[j], label = r'$\lambda$', linestyle = 'dashed', color = 'brown')
...
for string, [p0, p1, f0, f1] in dict_cases.items():
    ...
    for pi in listPi:
        #Mathematical models
        mathModelMFG = ModelCoupledFBSDE(T, QAver, R0,  jumpFactor, alpha, beta, coeffOU, A, K, pi, p0, p1, f0, f1, theta,C, S0, h1, h2,sig0, sig, alphaTarget, jumpModel, 1)
        mathModelMFCagg = ModelCoupledFBSDE(T, QAver, R0,  jumpFactor, alpha, beta, coeffOU, A, K, pi, p0, p1, f0, f1, theta,C, S0, h1, h2,sig0, sig, alphaTarget, jumpModel, 2)
